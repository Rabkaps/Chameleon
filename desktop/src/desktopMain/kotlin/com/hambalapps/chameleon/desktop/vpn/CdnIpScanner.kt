...
object CdnIpScanner {
    ...
    fun getCleanIp(
        ...
        if (pinnedIp.isNotEmpty()) {
            println("[Chameleon] Using pinned clean IP for $preset: $pinnedIp")
            return pinnedIp
        }

        val cached = cleanIpCache[preset]
        if (cached != null && (System.currentTimeMillis() - cached.second) < CACHE_DURATION_MS) {
            println("[Chameleon] Using cached clean IP for $preset: ${cached.first}")
            return cached.first
        }

        println("[Chameleon] No cached IP for $preset. Running clean IP scan...")
