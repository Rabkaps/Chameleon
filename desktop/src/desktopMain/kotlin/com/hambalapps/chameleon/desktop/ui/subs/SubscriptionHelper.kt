...
import com.hambalapps.chameleon.desktop.vpn.tryBase64Decode
...
fun parseDataSize(text: String): Long? {
    val clean = text.trim()
...
fun parseDateString(text: String): Long? {
    val clean = text.trim()
...
suspend fun fetchSubscription(urlStr: String): FetchResult = withContext(Dispatchers.IO) {
            ...
            if (connection.responseCode == 200) {
                ...
                if (decoded.trim().startsWith("{") && decoded.contains("\"servers\"")) {
                    try {
                        ...
                        val serverArray = jsonObj.optJSONArray("servers")
                        if (serverArray != null) {
                            for (i in 0 until serverArray.length()) {
                                val item = serverArray.optJSONObject(i) ?: continue
                                val host = item.optString("server")
                                val port = item.optInt("port", 443)
                                val remark = item.optString("remarks", "SIP008 Node")
                                if (host.isNotEmpty()) {
                                    servers.add("ss://${item.optString("password")}@$host:$port#$remark")
                                }
                            }
                        }
                    } catch (e: Exception) {}
                ...
                // Case B: Standard line-by-line URI list
                ...
                if (servers.isNotEmpty() || uploadVal != null || downloadVal != null) {
